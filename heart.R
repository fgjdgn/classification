...
# Create a prediction object using the predicted probabilities and actual labels
pred_obj <- prediction(pred[, 1], heart_test$fatal_mi)

# Create the ROC curve and plot it
roc_obj <- performance(pred_obj, "tpr", "fpr")
plot(roc_obj, main = "ROC Curve", xlab = "1- Specificity", ylab = "Sensitivety", col = "blue")
auc <- performance(pred_obj, "auc")@y.values[[1]]

# Label the AUC on the plot
text(0.6, 0.2, paste0("AUC = ", round(auc, 3)), cex = 1.2, col = "red")
